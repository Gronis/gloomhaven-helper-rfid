...
using namespace ghh::protocol::v8_4;
...
void ghh::protocol::v8_4::readGameState(GameState &state, Buffer &buffer) {
  ...
  buffer.readEnumArray(state.attack_modifiers_discard,
                       ghh::getAttackModifierValues());
...
static void __writePlayerActor(Buffer &buffer, const Actor &actor) {
  ...
  buffer.writeString(
      player.name == "nameless" ? tl::nullopt : tl::make_optional(player.name));
...
void ghh::protocol::v8_4::writeGameState(const GameState &state,
                                         Buffer &buffer) {
  ...
  buffer.writeBoolean(state.jotl);
  ...
  for (const auto deck_pair : state.ability_decks) {
    ...
    for (int i = 0, n = buffer.writeArrayStart(deck.abilities_discard); i < n;
         i++) {
