...
int main()
{
    ...
    auto write_network_data = [&](uint8_t value) {
        std::cout << std::hex << (int)value;
        std::cout << " ";
        send(sock, &value, 1, 0);
        return value;
    ...
    ghh::OutputStream output(write_dummy_data, output_buffer, bufferCapacity);
    ...
    auto send_game_state = [&]() {
        ...
        ghh::protocol::v7_6::writeGameState(state, msg);
    ...
    client.on_data = [&](std::string event, std::string payload, uint8_t *data, std::size_t dataLength) {
        ...
        if (event[0] == 's')
        {
            ...
            ghh::protocol::v7_6::readGameState(state, msg);
        }
    };
    ...
    while (true)
    {
        if (current >= end)
        {
            ...
            ghh::print("Read network: ", end, ", ", current, "\n");
        }
        client.update();
